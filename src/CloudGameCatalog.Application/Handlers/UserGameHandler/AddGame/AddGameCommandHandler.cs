...
public sealed class AddGameCommandHandler(
    ...
    IRedisCacheService redisCacheService) : IHandler<AddGameCommand, AddGameCommandResponse>
{    
    ...
    private async Task<User?> GetUserWithCacheAsync(int userId)
    {
        ...
        var cached = await redisCacheService.GetAsync<User>(cacheKey);
        ...
        if (user is not null)
            await redisCacheService.SetAsync(cacheKey, user, TimeSpan.FromMinutes(15));
